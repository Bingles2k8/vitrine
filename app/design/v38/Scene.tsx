...
/**
 * An aisle of racking that does not end. Domain repetition gives a hundred
 * bays for the price of one, and a hash per bay picks the object standing in
 * it — so no two bays hold the same thing.
 */
const FRAG = `${CORE}
...
vec2 map(vec3 p){
  ...
  // One object per bay: which shape, which side, which shelf, all from the
  // bay index. Deterministic, so it does not shimmer as the camera travels.
...
vec3 LAMP = vec3(1.00, 0.94, 0.84);
vec3 VOID = vec3(0.004, 0.005, 0.006);

vec3 shade(vec3 p, vec3 n, vec3 rd, float m, float bounce){
  vec3 alb; float rough; float f0;
  if (m < 1.5)      { alb = vec3(0.115, 0.12, 0.12); rough = 0.28; f0 = 0.05; }  // aisle floor
  else if (m < 3.5) { alb = vec3(0.30, 0.32, 0.31);  rough = 0.30; f0 = 0.10; }  // racking
  else              { alb = vec3(0.66, 0.65, 0.62);  rough = 0.24; f0 = 0.09; }  // objects
  ...
  col += alb * vec3(0.014, 0.016, 0.020) * occ;
  col += envSheen(n, rd, vec3(0.055, 0.058, 0.068), vec3(0.014, 0.013, 0.012), f0)
...
void main(){
  ...
  vec3 rd = camRay(uCam, uTarget, uv, ar > 1.0 ? 1.35 : 1.05);
  ...
  if (hit.x <= 36.0) {
    ...
    if (hit.y < 1.5) {
      ...
      vec2 h2 = march(ro, rr);
      vec3 q  = ro + rr * h2.x;
      vec3 rc = h2.x > 12.0 ? VOID : shade(q, normal(q), rr, h2.y, 1.0);
      col += rc * fresnel(n, rd, 0.05) * exp(-h2.x * 0.30);
    }

    // The aisle has to run out of light, not out of geometry.
    col *= exp(-hit.x * 0.085);
  }
  ...
  if (failed) {
    return (
      <div
        ...
        style={{ background: 'radial-gradient(46% 40% at 50% 34%, #3a352a 0%, #050506 76%)' }}
      />
    )
  }
