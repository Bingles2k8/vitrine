...
export default function ValuationPage() {
  ...
  const mediumOptions = Array.from(new Set(valuations.map(v => v.objects?.medium).filter(Boolean))).sort() as string[]
  const objectTypeOptions = Array.from(new Set(valuations.map(v => v.objects?.object_type).filter(Boolean))).sort() as string[]
  const artistOptions = [] as string[]

  const q = searchQuery.trim().toLowerCase()
  const filteredValuations = valuations
    .filter(v => {
      if (filters.dateFrom && (v.valuation_date || '') < filters.dateFrom) return false
      if (filters.dateTo && (v.valuation_date || '') > filters.dateTo) return false
      if (filters.medium && v.objects?.medium !== filters.medium) return false
      if (filters.objectType && v.objects?.object_type !== filters.objectType) return false
      if (filters.status && v.objects?.status !== filters.status) return false
      if (filters.accessionStatus === 'confirmed' && !v.objects?.accession_register_confirmed) return false
      if (filters.accessionStatus === 'unconfirmed' && v.objects?.accession_register_confirmed) return false
      if (filters.acquisitionMethod && v.objects?.acquisition_method !== filters.acquisitionMethod) return false
      if (!q) return true
      return (
