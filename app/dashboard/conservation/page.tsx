    ...
    <div className="min-h-screen bg-stone-50 flex">
      ...
      <main className="ml-56 flex-1 flex flex-col">
        <div className="h-14 border-b border-stone-200 bg-white flex items-center px-8 sticky top-0">
          <span className="font-serif text-lg italic text-stone-900">Conservation Register</span>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          {filtered.length === 0 ? (
            <div className="bg-white border border-stone-200 rounded-lg flex flex-col items-center justify-center py-24 text-center">
              ...
              <div className="font-serif text-2xl italic text-stone-900 mb-2">No conservation treatments recorded</div>
              <p className="text-sm text-stone-400">Open an object and go to the Conservation tab to record a treatment.</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-stone-50 border-b border-stone-200">
                    ...
                    <th className="text-left text-xs uppercase tracking-widest text-stone-400 font-normal px-4 py-3">Treatment Type</th>
                    ...
                    <th className="text-left text-xs uppercase tracking-widest text-stone-400 font-normal px-4 py-3">End</th>
                    ...
                    <tr key={t.id} className="border-b border-stone-100 hover:bg-stone-50">
                      <td className="px-6 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded bg-stone-100 flex items-center justify-center text-base">{t.artifacts?.emoji}</div>
                          <div>
                            <div className="text-sm font-medium text-stone-900">{t.artifacts?.title}</div>
                            <div className="text-xs font-mono text-stone-400">{t.artifacts?.accession_no}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm text-stone-700">{t.treatment_type}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-3">
                          {t.status === 'Active' && (
                            <>
                              <button onClick={() => updateStatus(t.id, 'Completed')} className="text-xs font-mono text-stone-400 hover:text-emerald-700 transition-colors">Complete</button>
                              <button onClick={() => updateStatus(t.id, 'Cancelled')} className="text-xs font-mono text-stone-400 hover:text-red-500 transition-colors">Cancel</button>
                            </>
                          )}
                          <button onClick={() => router.push(`/dashboard/artifacts/${t.artifact_id}?tab=conservation`)} className="text-xs font-mono text-stone-400 hover:text-stone-900 transition-colors">View →</button>
                        </div>
                      </td>
                    </tr>
                  ))}
