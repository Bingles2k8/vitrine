...
export default function DamagePage() {
  ...
  if (loading) return (
    <DashboardShell museum={null} activePath="/dashboard/damage" onSignOut={() => {}}>
    ...
    return (
      <DashboardShell museum={museum} activePath="/dashboard/damage" onSignOut={handleSignOut} isOwner={isOwner} staffAccess={staffAccess}>
          <div className="h-14 border-b border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-950 flex items-center px-4 md:px-8 sticky top-0">
            <span className="font-serif text-lg italic text-stone-900 dark:text-stone-100">Damage Reports</span>
          </div>
          <div className="flex-1 flex items-center justify-center p-8">
            <div className="text-center max-w-sm">
              <div className="text-5xl mb-5">◉</div>
              <h2 className="font-serif text-2xl italic text-stone-900 dark:text-stone-100 mb-3">Damage Reports is a Professional feature</h2>
              <p className="text-sm text-stone-400 dark:text-stone-500 mb-6">Document and track damage to collection items. Available on Professional, Institution, and Enterprise plans.</p>
              <button
  ...
  const filtered = reports
    .filter(r => {
      ...
      if (filters.dateFrom && (r.incident_date || '') < filters.dateFrom) return false
      if (filters.dateTo && (r.incident_date || '') > filters.dateTo) return false
            ...
            <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-lg overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {filtered.map(r => (
                    <tr key={r.id}
                      className={`border-b border-stone-100 dark:border-stone-800 hover:bg-stone-50 dark:hover:bg-stone-800 ${r.object_id ? 'cursor-pointer' : ''}`}
                      onClick={() => r.object_id && router.push(`/dashboard/objects/${r.object_id}?tab=damage`)}
                    >
