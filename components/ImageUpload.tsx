    ...
    <div>
      ...
      <div className="relative">
        {preview ? (
          ...
          <label className="flex flex-col items-center justify-center w-full aspect-square border-2 border-dashed border-stone-200 rounded-lg cursor-pointer hover:border-stone-400 transition-colors bg-stone-50">
            <div className="text-3xl mb-3">📷</div>
            <div className="text-sm text-stone-400 mb-1">
              {uploading ? 'Uploading...' : 'Upload a photo'}
            </div>
            <div className="text-xs text-stone-300">JPG, PNG, WEBP up to 10MB</div>
            <input
              type="file"
              accept="image/*"
              onChange={handleFile}
        ...
        {uploading && (
          <div className="absolute inset-0 flex items-center justify-center bg-white/70 rounded-lg">
            <div className="text-xs font-mono text-stone-500">Uploading...</div>
          </div>
        )}
      </div>
    </div>
