...
export default function Sidebar({ museum, activePath, onSignOut, isOwner = true, staffAccess = null, onNavigate }: SidebarProps) {
  ...
  async function handleExport() {
    ...
    const [
      ...
      fetchAll(r => supabase.from('objects').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('staff_members').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('entry_records').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('loans').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('conservation_treatments').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('audit_records').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('object_exits').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('locations').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('location_history').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('condition_assessments').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('valuations').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('risk_register').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('emergency_plans').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('insurance_policies').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('damage_reports').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('reproduction_requests').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('collection_use_records').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('disposal_records').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('collection_reviews').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('audit_exercises').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('rights_records').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('documentation_plans').select('*').eq('museum_id', mid).range(r.from, r.to)),
      fetchAll(r => supabase.from('documentation_plan_backlogs').select('*').eq('museum_id', mid).range(r.from, r.to)),
    ])
    const exportData = {
